...
When nothing else helps, this option could be tried to avoid stalls at PCI Configuration Begin phase of macOS booting by attempting to fix the ACPI addresses. It is not a magic bullet however, and only works with the most typical cases. Do not use unless absolutely required as it can have the opposite effect on certain platforms and result in boot failures.</source>
        <oldsource>RebaseRegions
Type: plist boolean
Failsafe: false
Description: Attempt to heuristically relocate ACPI memory regions. Not recommended.

ACPI tables are often generated dynamically by the underlying firmware implementation. Among the positionindependent code, ACPI tables may contain the physical addresses of MMIO areas used for device configuration,
typically grouped by region (e.g. OperationRegion). Changing firmware settings or hardware configuration,upgrading or patching the firmware inevitably leads to changes in dynamically generated ACPI code, which
sometimes results in the shift of the addresses in the aforementioned OperationRegion constructions.

For this reason, the application of modifications to ACPI tables is extremely risky. The best approach is to make as few changes as possible to ACPI tables and to avoid replacing any tables, particularly DSDT tables. When this
cannot be avoided, ensure that any custom DSDT tables are based on the most recent DSDT tables or attempt to remove reads and writes for the affected areas.

When nothing else helps, this option could be tried to avoid stalls at PCI Configuration Begin phase of macOS booting by attempting to fix the ACPI addresses. It is not a magic bullet however, and only works with the most
typical cases. Do not use unless absolutely required as it can have the opposite effect on certain platforms and result in boot failures.</oldsource>
        <translation>RebaseRegions
类型：plist boolean
失败保护：假
...
当没有其他帮助时，可以尝试这个选项，通过尝试修复ACPI地址来避免在macOS启动的PCI配置开始阶段的停顿。然而，这并不是灵丹妙药，只对最典型的情况有效。除非绝对需要，否则不要使用，因为它在某些平台上可能会产生相反的效果，导致启动失败。</translation>
    </message>
    <message>
...
waking from hibernation.</oldsource>
        <translation>ResetHwSig 
类型：plist boolean 
失败保护：假 

说明：将FACS表HardwareSignature值重置为0。
...
Note: Most but Apple and VMware firmwares need this quirk.</oldsource>
        <translation>AvoidRuntimeDefrag
...
it can also be used to disable this kext in other situations (e.g. with Cpuid1Data left blank).</source>
        <translation type="unfinished">注1：对于在macOS中没有本机电源管理驱动程序的CPU，此选项是NullCpuPowerManagement.kext的首选替代方案。
...
AppleRtcRam protocol description if this is desired.</source>
        <translation type="unfinished">注1：此选项不会保护其他区域不被覆盖，如果需要，请参阅RTCMemoryFixup内核扩展。 
...
utility creates EnableTRIM variable in APPLE_BOOT_VARIABLE_GUID namespace with 01 00 00 00 value.</source>
        <translation type="unfinished">注意：可以根据用户喜好避免使用此选项。 NVMe SSD兼容，无需更改。 
...
• x86_64 — Use x86_64 (64-bit) kernel when available.</source>
        <translation type="unfinished">在macOS 10.7和更早版本上，XNU内核可以使用不同于通常的x86_64的体系结构进行引导。 
...
• NVRAM — Use NVRAM detection.</source>
        <translation type="unfinished">休眠检测模式。
...
• Shift — safe mode.</source>
        <translation type="unfinished">除了PickerMode部分中部分描述的动作热键（通常由Apple BDS处理）之外，还有修饰键，这些键由操作系统的引导加载程序（boot.efi）处理。这些键允许通过提供不同的引导模式来更改操作系统行为。在某些固件上，由于驱动程序不兼容，使用修饰键可能会出现问题。要变通解决此问题，此选项允许从引导选择器中以更宽松的方式注册选择的热键。这样的扩展包括除了按住并按住Shift键以及其他键以外，还可以点击其他键，而不是仅按住Shift键，这在许多PS / 2键盘上是无法检测到的。
...
is set to true the behaviour is undefined when any of the fields are present in NVRAM section.</source>
        <translation>这些字段是从“常规”或“平台NVRAM”部分读取的，具体取决于“Automatic”值。 
...
EMI-reduction circuit as described in Acidanthera Bugtracker.</source>
        <translation>该值包含CPU ART频率，也称为晶体时钟频率。 它的存在是Skylake或更高版本独有的。 
...
modern Macs.</source>
        <translation>设置gEfiMiscSubClassGuid中的DevicePathsSupported。 
...
Note: While this quirk is required to run older macOS versions on platforms with used lower memory, it is not compatible with some hardware and macOS 11. In such cases, consider using EnableSafeModeSlide instead.</source>
        <translation>AllowRelocationBlock
...
• 0x0D — SODIMM
• 0x0F — FB-DIMM</source>
        <translation>指定内存的尺寸。 在Mac上，通常应为DIMM或SODIMM。 
...
does not affect them. Unlike SMBIOS they are always stored in the Big Endian format.</source>
        <translation>每个UUID AABBCCDD-EEFF-GGHH-IIJJ-KKLLMMNNOOPP基本上是一个16字节的十六进制数字。它可以用两种方式进行编码。
...
This option requires ProvideConsoleGop to be enabled. (TODO)</oldsource>
        <translation>有些类型的固件没有实现GOP协议，这个选项通过基于UGA的代理来提供。该选项需要启用 ProvideConsoleGop。</translation>
    </message>
    <message>
