...
MainWindow::MainWindow(QWidget *parent)
  ...
  title = "QtOpenCoreConfigurator   V0.6.4-2020.11.20";
  ...
  QString tabBarStyle0 =
      "QTabBar::tab {min-width:100px;border: 0px solid;border-top-left-radius: 5px;border-top-right-radius: 5px;padding:2px;}\
  ...
  QString tabBarStyle1 =
      "QTabBar::tab {min-width:100px;color: rgba(150,150,150);border: 2px solid;border-top-left-radius: 10px;border-top-right-radius: 10px;padding:5px;}\
            QTabBar::tab:!selected {margin-top: 5px;} \
            QTabBar::tab:selected {color: rgba(255,255,255);}";

  QString tabBarStyle2 =
      "QTabBar::tab {min-width:100px;color: black;background-color:qlineargradient(x1:0, y1:0, x2:0, y2:1, stop: 0 #eeeeee, stop: 1 gray);border: 0px solid;border-top-left-radius: 10px;border-top-right-radius: 10px;padding:5px;}\
            QTabBar::tab:!selected {margin-top: 5px;} \
...
void MainWindow::write_ini(QString table_name, QTableWidget *mytable, int i) {
  QString plistPath = QDir::homePath() + "/" + CurrentDateTime + table_name +
                      QString::number(i + 1) + ".ini";
  // qDebug() << plistPath;
...
void MainWindow::write_value_ini(QString tablename, QTableWidget *subtable,
  ...
  QString plistPath = QDir::homePath() + "/" + CurrentDateTime + tablename +
                      QString::number(i + 1) + ".ini";
  // qDebug() << plistPath;
  QFile file(plistPath);
  if (file.exists()) //如果文件存在，则先删除它
...
QVariantMap MainWindow::SaveBooter() {
  ...
  mapQuirks["EnableWriteUnprotector"] =
      getChkBool(ui->chkEnableWriteUnprotector);
  ...
  mapQuirks["SyncRuntimePermissions"] =
      getChkBool(ui->chkSyncRuntimePermissions);
...
QVariantMap MainWindow::SaveKernel() {
  ...
  mapQuirks["PowerTimeoutKernelPanic"] =
      getChkBool(ui->chkPowerTimeoutKernelPanic);
  mapQuirks["ThirdPartyDrives"] = getChkBool(ui->chkThirdPartyDrives);
  mapQuirks["XhciPortLimit"] = getChkBool(ui->chkXhciPortLimit);
  mapQuirks["DisableLinkeditJettison"] =
      getChkBool(ui->chkDisableLinkeditJettison);
...
QVariantMap MainWindow::SaveMisc() {
  ...
  // Boot
  valueList["ConsoleAttributes"] =
      ui->editConsoleAttributes->text().toLongLong();
  ...
  valueList["ExposeSensitiveData"] =
      ui->editExposeSensitiveData->text().toLongLong();
...
QVariantMap MainWindow::SavePlatformInfo() {
  ...
  valueList["DevicePathsSupported"] =
      ui->editDevicePathsSupported->text().toLongLong();
  ...
  valueList["StartupPowerEvents"] =
      ui->editStartupPowerEvents->text().toLongLong();
  ...
  valueList["SystemMemoryStatus"] =
      ui->cboxSystemMemoryStatus->currentText().trimmed();
  valueList["ProcessorType"] =
      ui->editProcessorTypeGeneric->text().toLongLong();
  ...
  valueList["FirmwareFeatures"] =
      HexStrToByte(ui->editFirmwareFeatures->text());
  valueList["FirmwareFeaturesMask"] =
      HexStrToByte(ui->editFirmwareFeaturesMask->text());
  ...
  valueList["FirmwareFeatures"] =
      HexStrToByte(ui->editFirmwareFeatures_2->text());
  valueList["FirmwareFeaturesMask"] =
      HexStrToByte(ui->editFirmwareFeaturesMask_2->text());
...
QVariantMap MainWindow::SaveUEFI() {
  ...
  dictList["KeyForgetThreshold"] =
      ui->editKeyForgetThreshold->text().toLongLong();
  dictList["KeyMergeThreshold"] =
      ui->editKeyMergeThreshold->text().toLongLong();
  ...
  // Output
  dictList.clear();
  dictList["ClearScreenOnModeSwitch"] =
      getChkBool(ui->chkClearScreenOnModeSwitch);
  ...
  dictList["AppleUserInterfaceTheme"] =
      getChkBool(ui->chkAppleUserInterfaceTheme);
  ...
  dictList["IgnoreInvalidFlexRatio"] =
      getChkBool(ui->chkIgnoreInvalidFlexRatio);
  ...
  dictList["ExitBootServicesDelay"] =
      ui->editExitBootServicesDelay->text().toLongLong();
...
void MainWindow::on_btnDPDel_Del0_clicked() {
  ...
  QString qz = QDir::homePath() + "/" + CurrentDateTime +
...
void MainWindow::on_btnACPIAdd_Add_clicked() {
  ...
  QStringList FileName =
      fd.getOpenFileNames(this, "文件", "", "文件(*.aml);;所有文件(*.*)");
  if (FileName.isEmpty())
...
void MainWindow::on_btnNVRAMLS_Del0_clicked() {
  ...
  QString qz =
      QDir::homePath() + "/" + CurrentDateTime + ui->table_nv_ls0->objectName();
  QFile file(qz + QString::number(delindex + 1) + ".ini");
...
void MainWindow::on_btnUEFIDrivers_Add_clicked() {
  ...
  QStringList FileName =
      fd.getOpenFileNames(this, "文件", "", "文件(*.efi);;所有文件(*.*)");
  if (FileName.isEmpty())
...
void MainWindow::init_tr_str() {
  ...
  strExecutablePath =
      tr("Kext executable path relative to bundle (e.g. Contents/MacOS/Lilu).");
...
void MainWindow::ScanPolicy() {
  ...
  int total = v1 + v2 + v3 + v4 + v5 + v6 + v7 + v8 + v9 + v10 + v11 + v12 +
              v13 + v14 + v15 + v16;
...
#ifdef Q_OS_MAC
  chkdata->start(appInfo.filePath() + "/ocvalidate", QStringList()
                                                         << SaveFileName);
#endif
